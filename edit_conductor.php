...
<?php
  $query=mysql_query("select * from driver where conductorid='$ID'")or die(mysql_error());
$row=mysql_fetch_array($query);
  ?>
				
				<p>Edit conductor account:</p>
				<div class="field">
			...
			<div class="login-actions">
				...
				<a href="conductor.php"><button type="button" style="margin-right:20px;" class="button btn btn-primary btn-large">Cancel</button></a>
			</div> 
			<!-- .actions -->
...
mysql_query(" UPDATE conductorr SET profile='$profile' WHERE conductorid = '$id' ")or die(mysql_error());
echo "<script>alert('Successfully Update Conductor Profile!'); window.location='conductor.php'</script>";
										}
										}
							}*/
?>
...
mysql_query(" UPDATE conductor SET firstname='$firstname', profile='$still_profile', lastname='$lastname', contact_number='$contact_number'
 WHERE conductorid = '$id' ")or die(mysql_error());
echo "<script>alert('Successfully Update Conductor Info!'); window.location='conductor.php'</script>";	
									}else{
										if($size > 10000000) //conditions for the file
										{
										die("Format is not allowed or file size is too big!");
...
mysql_query(" UPDATE conductor SET firstname='$firstname', profile='$profile', lastname='$lastname', contact_number='$contact_number'
 WHERE conductorid = '$id' ")or die(mysql_error());
echo "<script>alert('Successfully Update Conductor Info!'); window.location='conductor.php'</script>";
}
}
